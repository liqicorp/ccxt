...
class liqi(Exchange):
    ...
    def describe(self):
        return self.deep_extend(super(liqi, self).describe(), {
            'id': 'liqi',
            'name': 'Liqi',
    ...
    async def fetch_currencies(self, limit, params={}):
        request = {
            'limit': limit or 1000,
        }
    ...
    async def fetch_order(self, id, params={}):
        if id is None:
            raise ArgumentsRequired(' fetchOrder() precisa do id da ordem como parâmetro')
        request = {
            'id': id,
        }
    ...
    async def fetch_orders(self, symbol, limit, since=None, params={}):
        if symbol is None:
            raise ArgumentsRequired(' fetchOrders() precisa do symbol como parâmetro')
        request = {
            'symbol': symbol,
            'limit': limit or 50,
        }
    ...
    async def fetch_tickers(self, limit, params={}):
        request = {
            'limit': limit or 1000,
        }
    ...
    async def fetch_ohlcv(self, symbol, internal='1m', limit=500, params={}):
        request = {
            'symbol': symbol,
            'interval': internal,
            'limit': limit,
    ...
    async def create_order(self, symbol, type, side, amount=None, price=None, quoteAmount=None, params={}):
        request = {
            'symbol': symbol,
            'type': type,
            'side': side,
    ...
    async def fetch_open_orders(self, symbol=None, since=None, limit=None, params={}):
        if symbol is None:
            raise ArgumentsRequired(' fetchOpenOrders() precisa do symbol como parâmetro')
        request = {
            'symbol': symbol,
            'limit': limit or 50,
        }
    ...
    def sign_ecdsa(self, request, secret):
        signature = ed25519.sign(request, secret)
        signatureStr = self.to_der(signature, 'hex')
        return signatureStr

    def to_der(self, signature, enc):
        ...
        while(not s[0] and not (s[1] & 0x80)):
            s = s[1:]
        arr = [0x02]
        self.construct_length(arr, len(r))
        arr = arr.concat(r)
        arr.append(0x02)
        self.construct_length(arr, len(s))
        backHalf = arr.concat(s)
        res = [0x30]
        self.construct_length(res, len(backHalf))
        res = res.concat(backHalf)
        return utils.encode(res, enc)

    def construct_length(self, arr, len):
        if len < 0x80:
            arr.append(len)
            return
        octets = 1 + (math.log(len) / Math.LN2 >>> 3)
        arr.append(octets | 0x80)
        while(--octets):
            arr.append((len >>>(octets << 3)) & 0xff)
        arr.append(len)

    def rm_padding(self, buf):
        i = 0
        len = len(buf) - 1
    ...
    def sign(self, path, api='public', method='GET', params={}, headers=None, body=None):
        ...
        if api == 'private':
            ...
            hash = self.hash(query, 'sha256', 'hex')
            signature = self.sign_ecdsa(hash, self.secret)
            headers = {
                'signature': signature,
