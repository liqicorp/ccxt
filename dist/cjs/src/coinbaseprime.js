...
// ---------------------------------------------------------------------------
// ---------------------------------------------------------------------------
class coinbaseprime extends coinbaseprime$1 {
    constructor() {
        super(...arguments);
        this.portfolioId = '';
    }
    ...
    async fetchBalance(params = {}) {
        ...
        const response = await this.privateGetV1PortfoliosPortfolioIdBalances({ ...params, 'balance_type': 'TRADING_BALANCES' });
        return this.parseBalance(response.balances);
    }
    async loadPortfolio(params) {
        ...
        if (this.portfolioId === '') {
            const { portfolios } = await this.privateGetV1Portfolios(params);
            if (portfolios.length > 0) {
                this.portfolioId = portfolios[0].id;
            }
        }
        if (this.portfolioId !== '') {
            params['portfolio_id'] = this.portfolioId;
        }
    }
    parseBalance(response) {
        ...
        for (let i = 0; i < response.length; i++) {
            ...
            account['free'] = this.safeFloat(balance, 'withdrawable_amount');
    ...
    parseOrder(order, market = undefined) {
        ...
        market = this.safeMarket(marketId, market, '-');
        let status = this.parseOrderStatus(this.safeString(order, 'status').toLowerCase());
    ...
    async fetchOpenOrders(symbol = undefined, since = undefined, limit = undefined, params = {}) {
        ...
        const { orders } = await this.privateGetV1PortfoliosPortfolioIdOpenOrders(this.extend(request, params));
    ...
    async fetchOrders(symbol = undefined, since = undefined, limit = undefined, params = {}) {
        ...
        if (params['status']) {
            if (params['status'] === 'all') {
                ...
                request['order_statuses'] = [params['status'].toUpperCase()];
            }
        }
        const { orders } = await this.privateGetV1PortfoliosPortfolioIdOrders(this.extend(request, params));
        return this.parseOrders(orders, market, since, limit);
    }
    ...
    async createOrder(symbol, type, side, amount, price = undefined, params = {}) {
        ...
        const request = {
            ...
            'product_id': market['id'],
            'base_quantity': this.amountToPrecision(symbol, amount),
        ...
        if (clientOrderId !== undefined) {
            ...
            request['client_order_id'] = this.uuid();
        }
        ...
        params = this.omit(params, ['timeInForce', 'time_in_force', 'stopPrice', 'stop_price', 'client_order_id', 'postOnly', 'post_only']);
        if (type === 'limit') {
            request['limit_price'] = this.priceToPrecision(symbol, price);
            request['base_quantity'] = this.amountToPrecision(symbol, amount);
        }
        else if (type === 'market') {
    ...
    async cancelOrder(id, symbol = undefined, params = {}) {
        ...
        await this.loadPortfolio(params);
        const request = {
            'order_id': id, // the request will be more performant if you include it
        };
        await this.privatePostV1PortfoliosPortfolioIdOrdersOrderIdCancel(this.extend(request, params));
        return id;
    }
    ...
    sign(path, api = 'public', method = 'GET', params = {}, headers = undefined, body = undefined) {
        ...
        if (api === 'private') {
            ...
            const strToSign = this.buildPayload(nonce, method, requestPath, payload);
            const signature = this.cbSign(strToSign, this.secret);
            headers = {
                'X-CB-ACCESS-KEY': this.apiKey,
                'X-CB-ACCESS-SIGNATURE': signature,
                'X-CB-ACCESS-TIMESTAMP': nonce,
                'X-CB-ACCESS-PASSPHRASE': this.password,
    ...
    cbSign(str, secret) {
        const hash = CryptoJS__default["default"].HmacSHA256(str, secret);
        return hash.toString(CryptoJS__default["default"].enc.Base64);
    }
    // Function to build the payload required to sign
    buildPayload(ts, method, requestPath, body) {
